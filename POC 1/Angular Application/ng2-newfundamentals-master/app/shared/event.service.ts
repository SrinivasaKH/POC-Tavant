...
export class EventService {
    ...
    getEvent(id: number): Observable<IEvent> {
      ...
      // alert(id)
        // var ID = parseInt(atob(id.toString()));
    ...
    saveEvent(event): Observable<IEvent> {
        ...
        return this.http.post('http://localhost:62157/api/Authentication/AddNewEvent', JSON.stringify(event), options)
            .map((response: Response) => {
                console.log('test');
               // return response.json();
            }).catch(this.handleError);
    ...
    saveSession(session): Observable<ISession> {
        let headers = new Headers({ 'Content-Type': 'application/json' })
        let option = new RequestOptions({ headers: headers })
        return this.http.post('http://localhost:62157/api/Authentication/AddNewSession', session, option)
            .map((response: Response) => {
    ...
    searchSessions(searchTerm: string) {
        ...
        //     var matchSession = event.sessions.filter(session =>
